...
export default {
  ...
  methods: {
    ...
    movePrev(opts) {
      return this.move(-this.step_, opts, 'left');
    },
    moveNext(opts) {
      return this.move(this.step_, opts, 'right');
    },
    move(arg, opts = {}, direction='') {
      let currentMonth = this.pages[0].month
      let realMonth = new Date().getMonth() + 1
      console.log('真实月份：' + realMonth)
      console.log('当前月份：' + currentMonth)
      // 当前月份禁止右滑
      if (currentMonth == realMonth && direction == 'right') { return }
      // 上一月份禁止左滑
      if (realMonth == 1) {
        if (currentMonth == 12 && direction == 'left') { return }
      } else {
        if (currentMonth == realMonth-1 && direction == 'left') { return }
      }
